...
const uint16_t buffer_size = 255;
const char audiobuf[buffer_size];
...
void ELClientSocket::sendStream(File* data)
{
  _status = 0;
  if (remote_instance < 0) return;
  ...
  uint16_t chunkstosend = (filesize / buffer_size);
  if (filesize % buffer_size > 0) {
    chunkstosend++;
  }
  Serial.print(F("Chunks that should be send "));
  Serial.println(chunkstosend);
  _elc->Request(CMD_SOCKET_SEND, remote_instance, chunkstosend);
  int chunkssend=0;

  memset(audiobuf,0,buffer_size);
  data->seek(0);
  while (data->available()) {
    data->read(audiobuf, buffer_size);
    _elc->Request(audiobuf, sizeof(audiobuf));
    chunkssend++;
  }

  _elc->Request();
  Serial.print(F("Have sent "));
  Serial.print(chunkssend);
